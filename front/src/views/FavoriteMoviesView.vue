  ...
  <div class="wishlist-container">
    ...
    <div class="content-wrapper">
      <div class="header-section">
        <h1 class="page-title">찜한 영화 목록</h1>
      ...
      <div class="movies-grid" v-if="sortedMovies?.length">
        <div 
...
const { movieWishlist } = storeToRefs(wishStore)
...
const sortOptions = ref([
  { id: 1, name: '최신순', type: 'latest', active: true },
  { id: 2, name: '제목순', type: 'title', active: false },
]);

// 정렬된 영화 목록
const sortedMovies = computed(() => {
  const activeSort = sortOptions.value.find(opt => opt.active)
  if (!movieWishlist.value) return []
  
  const sorted = [...movieWishlist.value]
...
const handleWish = async (movie) => {
  if (confirm('찜 목록에서 삭제하시겠습니까?')) {
    try {
      await wishStore.wishMovie(movie.movie_id)
      await wishStore.getMovieWishlist()  // 목록 갱신
    } catch (err) {
